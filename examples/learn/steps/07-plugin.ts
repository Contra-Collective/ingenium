...
//    • `declare module 'riftexpress'` opts you into typed access to your
...
const auth: RiftexPlugin<{ token: string }> = (app, opts) => {
  app.decorate('user', (ctx) => {
    const header = ctx.headers.authorization ?? ''
    if (header !== `Bearer ${opts.token}`) {
      throw new RiftexUnauthorizedError('bring a valid token')
    }
