 ...
 *
 * These tests boot a real Ingenium app on an ephemeral port (NodeAdapter,
 ...
 *      `req.cookies` back into `ctx.state.cookies` so downstream Ingenium
 ...
 *      written explanation; users should rely on a future Ingenium-native
 ...
 *      instead — Ingenium already parses bodies natively.
 *
 ...
 *      session's wrapped end is never reached during a real IngeniumContext
 *      response (Ingenium writes the response itself after dispatch).
...
describe('compression (UNSUPPORTED)', () => {
   ...
   *
   * Recommendation:
   *   Use a Ingenium-native gzip middleware (TODO upstream). For static assets,
...
describe('cookie-parser (SUPPORTED)', () => {
  ...
  beforeAll(async () => {
    booted = await boot((app) => {
      ...
      // Downstream Ingenium middleware reads via ctx.state.cookies (mirrored from req.cookies).
...
describe('body-parser (UNSUPPORTED)', () => {
   ...
   *
   * Recommendation:
   *   USE `await ctx.body.json()` — Ingenium already parses request
   *   bodies natively, with the same 100kb default limit. body-parser
   *   is redundant in a Ingenium app.
   */
...
describe('passport (UNSUPPORTED for auth flows; initialize() is a no-op)', () => {
   ...
   *
   * Recommendation:
   *   Implement auth natively with IngeniumContext (`ctx.redirect`,
   *   `ctx.state.user`, Ingenium sessionMiddleware).
   */
...
describe('express-session (UNSUPPORTED)', () => {
   ...
   *   ran, Ingenium writes the response from ctx._body / ctx._headers
   ...
   *
   * Recommendation:
   *   Use Ingenium's native `sessionMiddleware` from
   *   `ingenium/session` — same surface, integrated correctly.
   */
  it.skip('would persist session via Set-Cookie', async () => {
    const booted = await boot((app) => {
      ...
      app.get('/s', (ctx: IngeniumContext) => ctx.json({ ok: true }))
    })
    try {
...
describe('multer (UNSUPPORTED)', () => {
   ...
   *
   * Recommendation:
   *   USE `await ctx.body.multipart()` — Ingenium has a native
  ...
  it.skip('would populate req.file from multipart upload', async () => {
    const booted = await boot((app) => {
      ...
      app.post('/upload', (ctx: IngeniumContext) => ctx.json({ ok: true }))
    })
    try {
