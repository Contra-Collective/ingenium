...
function makeCtx(init?: Partial<{ method: string; url: string; path: string; rawQuery: string; headers: Record<string, string> }>): IngeniumContext {
...
describe('expressCompat', () => {
  ...
  it('middleware that calls next() without writing lets the Ingenium chain continue', async () => {
    ...
    const downstream: IngeniumMiddleware = async (ctx) => {
      downstreamRan = true
...
describe('expressCompat — known-broken detection', () => {
  ...
  it('opt-out: { allowKnownBroken: true } emits a warning instead of throwing', () => {
    ...
    return new Promise<void>((resolve) => {
      setImmediate(() => {
        ...
        expect(warnings.every((w) => w.name === 'IngeniumCompatKnownBroken')).toBe(true)
  ...
  it('opt-out: returns a working IngeniumMiddleware (warning path does not short-circuit)', async () => {
