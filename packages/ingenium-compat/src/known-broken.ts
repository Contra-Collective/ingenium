 ...
 *
 * The list is kept in sync with `packages/ingenium-compat/COMPATIBILITY.md`
 * and the e2e suite at `packages/ingenium-compat/test/e2e.test.ts`.
 */
...
export interface KnownBrokenEntry {
  ...
  reason: string
  /** The Ingenium-native equivalent the user should reach for. */
  native: string
}
...
export const KNOWN_BROKEN: Record<string, KnownBrokenEntry> = {
  ...
  session: {
    ...
    native: "import { sessionMiddleware } from 'ingenium'",
  },
  compression: {
    pkg: 'compression',
    reason:
...
export function formatBrokenMessage(d: DetectedBroken): string {
  return (
    ...
    `Use Ingenium's native equivalent instead: \`${d.native}\`. ` +
    `See packages/ingenium-compat/COMPATIBILITY.md for the full matrix. ` +
