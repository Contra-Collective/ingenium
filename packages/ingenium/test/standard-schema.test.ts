...
const attach = (body: IngeniumBody, src: Readable | null) => body._attach(src, undefined, undefined)
const stream = (s: string) => Readable.from([Buffer.from(s)])

describe('Standard Schema support in IngeniumBody.json()', () => {
  ...
  it('throws IngeniumValidationError with multi-path fields on multiple issues', async () => {
    ...
    try {
      ...
      const fields = (err as IngeniumValidationError).fields
