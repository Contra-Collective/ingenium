...
function makeCtx(overrides: Partial<{ method: HttpMethod; path: string }> = {}): IngeniumContext {
...
function readJsonBody(ctx: IngeniumContext): unknown {
  const body = ctx._body
  if (body.kind !== 'string') throw new Error(`expected string body, got ${body.kind}`)
  return JSON.parse(body.data)
}

describe('problemDetails — IngeniumError serialization', () => {
  it('serializes IngeniumNotFoundError as a 404 Problem+JSON', async () => {
    const mw = problemDetailsMiddleware()
    const ctx = makeCtx({ method: 'GET', path: '/missing' })

    await mw(ctx, async () => { throw new IngeniumNotFoundError('Item gone') })
  ...
  it('typeBaseUrl shapes the type field with kebab-case slug', async () => {
    ...
    const ctx = makeCtx()
    await mw(ctx, async () => { throw new IngeniumNotFoundError() })
    const problem = readJsonBody(ctx) as Record<string, unknown>
  ...
  it('typeBaseUrl works without trailing slash', async () => {
    const mw = problemDetailsMiddleware({ typeBaseUrl: 'https://api.example.com/errors' })
    const ctx = makeCtx()
    await mw(ctx, async () => { throw new IngeniumValidationError({ email: 'invalid' }) })
    const problem = readJsonBody(ctx) as Record<string, unknown>
  ...
  it('IngeniumValidationError includes fields extension', async () => {
    const mw = problemDetailsMiddleware()
    const ctx = makeCtx({ method: 'POST', path: '/users' })
    await mw(ctx, async () => {
      throw new IngeniumValidationError({ email: 'must be a valid email', age: 'must be >= 18' })
    })
  ...
  it('IngeniumMethodNotAllowedError includes allowed array AND sets Allow header', async () => {
    const mw = problemDetailsMiddleware()
    const ctx = makeCtx({ method: 'DELETE', path: '/users/42' })
    await mw(ctx, async () => {
      throw new IngeniumMethodNotAllowedError(['GET', 'POST'])
    })
  ...
  it('uses custom instance callback', async () => {
    const instance = vi.fn((ctx: IngeniumContext) => `urn:request:${ctx.method}:${ctx.path}`)
