...
import { IngeniumCronJob } from '../src/cron/scheduler.ts'
...
describe('IngeniumCronJob', () => {
  it('runs the handler at the next scheduled tick', async () => {
    ...
    const job = new IngeniumCronJob('* * * * *', {}, (ctx) => {
  ...
  it('runOnStart fires immediately', async () => {
    ...
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-05-12T12:34:30Z'))
    const job = new IngeniumCronJob('0 0 1 1 *', { runOnStart: true }, () => { fired++ })
    job.start()
  ...
  it('overlap=skip drops a tick when previous run is still in flight', async () => {
    ...
    const job = new IngeniumCronJob('* * * * *', { overlap: 'skip' }, async () => {
      started++
      await blocker
    })
    job.start()
  ...
  it('overlap=queue queues exactly one pending run', async () => {
    ...
    let started = 0
    const releases: (() => void)[] = []
    const job = new IngeniumCronJob('* * * * *', { overlap: 'queue' }, async () => {
      started++
  ...
  it('stop() cancels the timer and prevents further fires', async () => {
    ...
    vi.setSystemTime(new Date('2026-05-12T12:00:30Z'))
    let fired = 0
    const job = new IngeniumCronJob('* * * * *', {}, () => { fired++ })
    job.start()
  ...
  it('rejects an invalid timezone at first nextFire computation', () => {
    expect(() => {
      const job = new IngeniumCronJob('* * * * *', { timezone: 'Mars/Olympus' }, () => {})
      job.start()
