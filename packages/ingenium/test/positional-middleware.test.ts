...
describe('positional middleware — IngeniumApp', () => {
  it('single inline middleware runs before the handler', async () => {
    const app = ingenium()
    const order: string[] = []
    const mw1: IngeniumMiddleware = async (_ctx, next) => {
      order.push('mw1')
      await next()
    }
    app.get('/', mw1, (ctx) => {
  ...
  it('multiple inline middleware run in declaration order', async () => {
    const app = ingenium()
    const order: string[] = []
    const mk = (name: string): IngeniumMiddleware => async (_ctx, next) => {
      order.push(name)
      await next()
    }
  ...
  it('inline mw can short-circuit (no next() → handler does not run)', async () => {
    ...
    const guard: IngeniumMiddleware = (ctx) => {
