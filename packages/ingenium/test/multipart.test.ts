...
const BOUNDARY = '----IngeniumTestBoundary'
const CT = `multipart/form-data; boundary=${BOUNDARY}`

const attach = (body: IngeniumBody, source: Readable | null, contentType?: string, contentLength?: number) => {
...
describe('IngeniumBody.multipart()', () => {
  ...
  it('rejects body exceeding maxBytes (413)', async () => {
    ...
    await expect(body.multipart({ maxBytes: 100 })).rejects.toBeInstanceOf(IngeniumPayloadTooLargeError)
  })
  ...
  it('rejects file exceeding maxFileSize (413)', async () => {
    ...
    ).rejects.toBeInstanceOf(IngeniumPayloadTooLargeError)
  })
  ...
  it('rejects too many files (400)', async () => {
    ...
    attach(body, fromBuf(buf), CT)
    await expect(body.multipart({ maxFiles: 2 })).rejects.toBeInstanceOf(IngeniumBadRequestError)
  })
  ...
  it('rejects too many fields (400)', async () => {
    ...
    attach(body, fromBuf(buf), CT)
    await expect(body.multipart({ maxFields: 2 })).rejects.toBeInstanceOf(IngeniumBadRequestError)
  })
  ...
  it('rejects disallowed mime type (400)', async () => {
    ...
    ).rejects.toBeInstanceOf(IngeniumBadRequestError)
  })
