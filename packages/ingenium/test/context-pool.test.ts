...
import { IngeniumContextPool } from '../src/context/pool.ts'
import { IngeniumContext } from '../src/context/context.ts'

describe('IngeniumContextPool', () => {
  it('acquire returns a fresh IngeniumContext when empty', () => {
    const pool = new IngeniumContextPool(8)
    const ctx = pool.acquire()
    expect(ctx).toBeInstanceOf(IngeniumContext)
  ...
  it('respects max size — overflow contexts are discarded', () => {
    const pool = new IngeniumContextPool(2)
  ...
  it('size getter reflects current free-list length', () => {
    const pool = new IngeniumContextPool(4)
    expect(pool.size).toBe(0)
  ...
  it('acquire after pool overflow still allocates fresh contexts', () => {
    const pool = new IngeniumContextPool(1)
    const a = pool.acquire()
    ...
    expect(b).toBe(a)
    expect(c).not.toBe(a)
    expect(c).toBeInstanceOf(IngeniumContext)
  })

  it('default constructor admits at least 10 released contexts', () => {
    const pool = new IngeniumContextPool()
