...
import { IngeniumHaltError } from '../src/errors.ts'
...
describe('ctx.halt — direct throw', () => {
  it('throws IngeniumHaltError synchronously', () => {
    const ctx = new IngeniumContext()
    expect(() => ctx.halt(401)).toThrow(IngeniumHaltError)
  })
  ...
  it('halt with no body sets bodyShape="none"', () => {
    ...
    try {
      ...
      const halt = err as IngeniumHaltError
...
describe('ctx.halt — error boundary serialization', () => {
  ...
  it('app.onError can intercept IngeniumHaltError and override', async () => {
    const app = ingenium()
