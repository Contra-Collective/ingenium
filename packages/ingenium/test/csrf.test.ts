...
function ctx(method: string, headers: Record<string, string | string[]> = {}, query = ''): IngeniumContext {
...
describe('csrfMiddleware (cookie storage)', () => {
  ...
  it('exposes the current token via ctx.state.csrfToken and ctx.csrfToken()', async () => {
    ...
    const fn = (c as IngeniumContext & { csrfToken: () => string }).csrfToken
  ...
  it('unsafe method without cookie throws IngeniumCsrfError (403)', async () => {
    const c = ctx('POST', { 'x-csrf-token': 'whatever' })
    await expect(mw(c, next)).rejects.toBeInstanceOf(IngeniumCsrfError)
  })

  it('unsafe method with cookie but no header throws', async () => {
    const tok = signed('abc123')
    const c = ctx('POST', { cookie: `ingenium.csrf=${encodeURIComponent(tok)}` })
    await expect(mw(c, next)).rejects.toBeInstanceOf(IngeniumCsrfError)
  })
  ...
  it('unsafe method with mismatched header throws', async () => {
    ...
    const c = ctx('POST', {
      cookie: `ingenium.csrf=${encodeURIComponent(tok)}`,
  ...
  it('rejects forged cookie without valid HMAC signature', async () => {
    ...
    const c = ctx('POST', {
      cookie: `ingenium.csrf=${encodeURIComponent(forged)}`,
      'x-csrf-token': forged,
    })
  ...
  it('accepts _csrf query string param as fallback', async () => {
    ...
    const c = ctx('POST', { cookie: `ingenium.csrf=${encodeURIComponent(tok)}` }, `_csrf=${encodeURIComponent(tok)}`)
    await expect(mw(c, next)).resolves.toBeUndefined()
  })
  ...
  it('appends Set-Cookie alongside other cookies (does not clobber)', async () => {
    ...
    expect(cookies[1]).toMatch(/^ingenium\.csrf=/)
  })
  ...
  it('accepts secret rotation: cookie signed with secrets[1] still verifies', async () => {
    ...
    const c = ctx('POST', {
      cookie: `ingenium.csrf=${encodeURIComponent(oldToken)}`,
...
describe('csrfMiddleware (session storage)', () => {
  function ctxWithSession(method: string, sessionData: Record<string, unknown> = {}): IngeniumContext {
    const c = ctx(method)
    let store = { ...sessionData }
    ;(c as IngeniumContext & { session: { get(k: string): unknown; set(k: string, v: unknown): void } }).session = {
      get: (k) => store[k],
      set: (k, v) => {
        store[k] = v
      },
...
describe('IngeniumCsrfError', () => {
  it('has the expected status, code, and message', () => {
    const err = new IngeniumCsrfError()
  ...
  it('serializes as a payload-shaped object', () => {
    const err = new IngeniumCsrfError('custom')
    // Mirrors what the default boundary does for any IngeniumError.
