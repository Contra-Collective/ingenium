...
import {
  ...
  type IngeniumContext,
  type Transport,
  type TransportHooks,
} from 'ingenium'
import type { HttpMethod } from 'ingenium'
 ...
 * core path so this adapter has zero coupling to Ingenium private modules.
 * Behaves identically: aborts the stream with `IngeniumPayloadTooLargeError`
 ...
 * request, populate a pooled `IngeniumContext` from the WinterCG `Request`,
...
function populateContext(ctx: IngeniumContext, req: Request, maxRequestBytes: number): void {
...
function buildResponse(ctx: IngeniumContext): Response {
