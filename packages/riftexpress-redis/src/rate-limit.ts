import type { RateLimitStore } from 'riftexpress'
 ...
 */
const HIT_SCRIPT = `-- RIFTEX_RATELIMIT_HIT v1
...
export interface RedisRateLimitStoreOptions {
  ...
  /** Key prefix for every entry. Default `'riftex:rl:'`. */
  prefix?: string
}
 ...
 * locally only to produce the absolute timestamp the rest of RiftExpress
...
export class RedisRateLimitStore implements RateLimitStore {
  ...
  constructor(opts: RedisRateLimitStoreOptions) {
    ...
    this.prefix = opts.prefix ?? 'riftex:rl:'
  }
