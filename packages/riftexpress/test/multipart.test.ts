...
import { RexBadRequestError, RexPayloadTooLargeError } from '../src/errors.ts'
import type { MultipartFile } from '../src/body/multipart-types.ts'

const BOUNDARY = '----RexTestBoundary'
const CT = `multipart/form-data; boundary=${BOUNDARY}`

const attach = (body: RexBody, source: Readable | null, contentType?: string, contentLength?: number) => {
...
describe('RexBody.multipart()', () => {
  ...
  it('rejects Content-Type without boundary', async () => {
    ...
    await expect(body.multipart()).rejects.toBeInstanceOf(RexBadRequestError)
  })
  ...
  it('rejects body exceeding maxBytes (413)', async () => {
    ...
    await expect(body.multipart({ maxBytes: 100 })).rejects.toBeInstanceOf(RexPayloadTooLargeError)
  })
  ...
  it('rejects file exceeding maxFileSize (413)', async () => {
    ...
    ).rejects.toBeInstanceOf(RexPayloadTooLargeError)
  })
  ...
  it('rejects too many files (400)', async () => {
    ...
    attach(body, fromBuf(buf), CT)
    await expect(body.multipart({ maxFiles: 2 })).rejects.toBeInstanceOf(RexBadRequestError)
  })
  ...
  it('rejects too many fields (400)', async () => {
    ...
    attach(body, fromBuf(buf), CT)
    await expect(body.multipart({ maxFields: 2 })).rejects.toBeInstanceOf(RexBadRequestError)
  })
  ...
  it('rejects disallowed mime type (400)', async () => {
    ...
    ).rejects.toBeInstanceOf(RexBadRequestError)
  })
