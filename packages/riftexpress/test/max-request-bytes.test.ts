 ...
 *
 * The cap configured via `riftex({ maxRequestBytes })` MUST fire BEFORE the
...
describe('NodeAdapter — maxRequestBytes', () => {
  ...
  describe('custom 500_000-byte ceiling', () => {
    let server: ListeningServer
    beforeAll(async () => {
      const app = riftex({ maxRequestBytes: 500_000 })
      app.post('/echo', async (ctx) => {
  ...
  describe('ctx.body.stream() consumers are also capped', () => {
    let server: ListeningServer
    beforeAll(async () => {
      const app = riftex({ maxRequestBytes: 50_000 })
  ...
  describe('maxRequestBytes: Infinity disables the cap', () => {
    let server: ListeningServer
    beforeAll(async () => {
      const app = riftex({ maxRequestBytes: Number.POSITIVE_INFINITY })
      app.post('/echo', async (ctx) => {
...
describe.skipIf(!hasBun)('BunAdapter — maxRequestBytes', () => {
  ...
  beforeAll(async () => {
    // Dynamic import keeps Node from choking on the `riftexpress-bun` package
    // when its `Bun` runtime guard would otherwise throw at construction time.
    const { BunAdapter } = (await import('../../riftexpress-bun/src/index.ts')) as {
