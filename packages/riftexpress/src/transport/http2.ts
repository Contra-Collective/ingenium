    ...
    if (this.options.allowHttp1 === true) {
      ...
      server.on('request', (req, res) => {
        handleHttp1Fallback(req, res, hooks).catch((err) => {
          ...
          process.emitWarning(`riftexpress(h2/http1): dispatch leaked: ${(err as Error).message ?? String(err)}`)
        })
      })
    }
...
function emergencyAbort(stream: ServerHttp2Stream, err: unknown): void {
  ...
  process.emitWarning(`riftexpress(h2): dispatch leaked: ${(err as Error).message ?? String(err)}`)
}
