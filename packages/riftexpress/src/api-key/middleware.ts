 ...
 * module-augment `RiftexContext` for typed access:
 *
 * @example
 * ```ts
 * declare module 'riftexpress' {
 *   interface RiftexContext { apiKey?: string }
 * }
 *
 * import { riftex } from 'riftexpress'
 * const app = riftex()
 * app.use(riftex.apiKey({
 ...
 * - The wire-facing error is always `RiftexUnauthorizedError('Invalid API key')`
 ...
 */
export function apiKeyMiddleware(opts: ApiKeyOptions): RiftexMiddleware {
  ...
  const logger: ApiKeyLogger =
    opts.logger ?? ((event) => process.emitWarning(`api-key auth failed: ${event.reason}`, 'RiftexApiKeyWarning'))
  ...
  return async (ctx, next) => {
    ...
    if (!key) {
      if (required) {
        ...
        throw new RiftexUnauthorizedError('Missing API key')
      }
      await next()
      return
    }
    ...
    try {
      ...
      throw new RiftexUnauthorizedError('Invalid API key')
    }
    if (!ok) {
    ...
    ;(ctx as RiftexContext & { apiKey?: string }).apiKey = key
    await next()
  }
}
