...
import { z } from 'zod'
import { IngeniumNotFoundError, IngeniumValidationError, Router } from 'ingenium'
...
export function notesRouter(db: DB): Router {
  ...
  router.get('/:id', (ctx) => {
    ...
    if (!row || row.user_id !== user.id) throw new IngeniumNotFoundError('Note not found')
    return toDto(db, row)
  })
  ...
  router.patch('/:id', async (ctx) => {
    ...
    if (!existing || existing.user_id !== user.id) throw new IngeniumNotFoundError('Note not found')
