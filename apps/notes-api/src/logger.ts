// Pino logger + a Ingenium plugin that decorates every ctx with a
...
export const loggerPlugin: IngeniumPlugin<{ logger: Logger }> = (app, opts) => {
  ...
  app.hooks.onResponse((ctx) => {
    // `_statusCode` is technically @internal in IngeniumContext but it's the only
